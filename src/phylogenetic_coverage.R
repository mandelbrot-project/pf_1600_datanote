...
if (config$params$download_pf_metadata == TRUE) {
  download.file(url = config$data$pf_metadata_url, destfile = paste0(config$paths$inputs, config$filenames$pf_metadata_local), method = "wget", extra = "-r -p --random-wait")
}

pf_metadata <- read_delim(paste0(config$paths$inputs, config$filenames$pf_metadata_local), delim = "\t", escape_double = FALSE, trim_ws = TRUE)
...
fam_mrca <- tapply(family, family, function(x) {
  return(tryCatch(MRCA(tree_plot, x), error = function(e) NULL))
})
...
colfunc <- colorRampPalette(c(
  "darkseagreen4", "orangered4", "slategray4", "palegreen3", "black",
...
x <- ggtree(my_tree, layout = "circular") +
  ...
  geom_cladelab(clades, names(clades),
    offset = 14, angle = (x1_fam$angle),
...
pdf(
  file = paste0(config$paths$outputs, config$filenames$family_coverage_plot), # The directory you want to save the file in
  width = 13, # The width of the plot in inches
  height = 8
)
...
pdf(
  file = paste0(config$paths$outputs, config$filenames$order_coverage_plot), # The directory you want to save the file in
  width = 13, # The width of the plot in inches
  height = 8
)
...
htmlwidgets::saveWidget(as_widget(p3_ly), paste0(config$paths$outputs, config$filenames$family_coverage_html))
...
htmlwidgets::saveWidget(as_widget(p4_ly), paste0(config$paths$outputs, config$filenames$order_coverage_html))
